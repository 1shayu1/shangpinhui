        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="features in spuSaleAttrList" :key="features.id">
                <dt class="title">选择{{ features.saleAttrName }}</dt>
                <!-- v-for="(featuresValue,index) in features.saleAttrName" -->
                <dd
                  changepirce="0"
                  v-for="featuresValue in features.spuSaleAttrValueList"
                  :key="featuresValue.id"
                  :class="{ active: featuresValue.isChecked == 1 }"
                  @click="
                    changeActive(featuresValue, features.spuSaleAttrValueList)
                  "
                >
                  {{ featuresValue.saleAttrValueName }}
                </dd>
              </dl>
              <!-- <dl>
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a href="javascript:" @click="addToShoppingCarts">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...mapActions('detail',["detailsPage", "addToShoppingCart"]),
    //属性选择高亮
    changeActive(featuresValue, arr) {
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      featuresValue.isChecked = 1;
    },
    add() {
      this.quantity += 1;
    },
    ...
    revise(e) {
      let nan = e.target.value * 1;
      if (isNaN(nan) || nan < 1) {
        this.quantity = 1;
      } else {
        this.quantity = parseInt(nan);
      }
    },
    addToShoppingCarts() {
      let add = this.addToShoppingCart({
        skuId: this.$route.params.skuId - 0,
      ...
      add.then(value=>{
        if(value.code==200){
          ...
          this.$router.push({
            name:'addCartSuccess',
            params:{quantity:this.quantity,skuId:this.$route.params.skuId}
          })
        }
      })
    },
  ...
  mounted() {
    //详情看商品详细接口
    this.detailsPage(this.$route.params.skuId);
  },
};
</script>
